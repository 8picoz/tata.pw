...
## IUnknown, Interface
ID3D12ResourceやID3D12RootSignatureなどのIUnknownな型は、windows-rsでいう[IUnknown](https://microsoft.github.io/windows-docs-rs/doc/windows/core/struct.IUnknown.html)ラップしてこれが実際のvtable(仮想関数テーブル)を保持してライフタイムを管理しています。  
...
## csoファイルの生成の自動化
Visual Studioでのビルドのようにシェーダー周りのビルドを最初から自動ではやってくれないのでビルドスクリプトを書いて自動化するようにしたいです。  


## パストレーシング
古典的レイトレーシングのみではなくパストレーシングなど他の発展的な手法に挑戦したいです。
